...
function initAudioOnceAfterUserGesture() {
  ...
  try {
    ...
    audio.sounds = {
      bgm: new Howl({
        ...
        volume: 0.3,
        onload: () => {
          audio.loaded.bgm = true;
        },
        onloaderror: () => {
          audio.loaded.bgm = false;
        },
      }),
      catch: new Howl({
        src: ["sound/catch.mp3"],
        volume: 0.6,
        onload: () => {
          audio.loaded.catch = true;
        },
        onloaderror: () => {
          audio.loaded.catch = false;
        },
      }),
      fail: new Howl({
        src: ["sound/fail.mp3"],
        volume: 0.6,
        onload: () => {
          audio.loaded.fail = true;
        },
        onloaderror: () => {
          audio.loaded.fail = false;
        },
      }),
      start: new Howl({
        src: ["sound/start.mp3"],
        volume: 0.5,
        onload: () => {
          audio.loaded.start = true;
        },
        onloaderror: () => {
          audio.loaded.start = false;
        },
      }),
      gameover: new Howl({
        ...
        volume: 0.7,
        onload: () => {
          audio.loaded.gameover = true;
        },
        onloaderror: () => {
          audio.loaded.gameover = false;
        },
      }),
    };
  } catch (error) {
...
function playSound(name, fallback) {
  if (state.muted) return;
  if (audio.enabled && audio.sounds[name] && audio.loaded[name] !== false) {
    try {
      audio.sounds[name].play();
      return;
    } catch (error) {
      audio.loaded[name] = false;
    }
  }
  if (fallback) {
    fallback();
...
function playBgm() {
  if (state.muted) return;
  if (audio.enabled && audio.sounds.bgm && audio.loaded.bgm !== false) {
    try {
...
function stopBgm() {
  if (audio.enabled && audio.sounds.bgm) {
    try {
      audio.sounds.bgm.stop();
    } catch (error) {
      audio.loaded.bgm = false;
    }
  }
...
function startGame() {
  ...
  playSound("start", () => beep(520, 0.12, "triangle"));
  playBgm();
}
...
function gameOver() {
  ...
  stopBgm();
  playSound("gameover", playGameOverSound);
}
...
function handleCatch(item) {
  if (item.type === "bomb") {
    ...
    if (item.type === "gold") {
      ...
      playSound("catch", () => beep(880, 0.18, "triangle"));
    } else {
      playSound("catch", () => beep(660, 0.12, "triangle"));
    }
    saveHighscore();
  }
  updateHud();
...
function handleMiss(item) {
  ...
  state.lives -= 1;
  playSound("fail", () => beep(180, 0.2, "sawtooth"));
